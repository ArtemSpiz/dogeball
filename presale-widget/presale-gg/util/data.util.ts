...
export type PaymentTokenList = {
  currencies: API.PaymentToken[];
  defaultToken?: API.PaymentToken;
  defaultLabel?: string;
  placeholder?: string;
  network?: string;
};

export const groupTokens = (tokens: API.PaymentToken[]): PaymentTokenList[] => {
  ...
  tokens.forEach((token) => {
    if (token.symbol.toLowerCase() === "card") {
      grouped["CARD"].tokens.push({ ...token, chain: "Visa/Mastercard/Etc" });
      return;
    }
    let hasToken = false;
    ...
    if (!hasToken) grouped["MORE"].tokens.push(token);
  });

  return Object.entries(grouped).map(([key, value]): PaymentTokenList => {
    const getSortValue = ([token, i]: [API.PaymentToken, number]) =>
      token.symbol.toUpperCase() === value.defaultTokenSymbol?.toUpperCase()
        ? -1
        : i;
    return {
      currencies: value.tokens
        .map((token, i) => [token, i] as [API.PaymentToken, number])
